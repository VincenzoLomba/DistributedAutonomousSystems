...
class AggregativeOptimizer:
    ...
    def run(self, max_iters=1000, step_size=0.01):
        ...
        for k in range(max_iters):
            ...
            prev_sigmas = [agent.sigma_estimate.copy() for agent in self.agents]
            ...
            for i, agent in enumerate(self.agents):
                new_sigma = agent.weights[agent.id] * prev_sigmas[i]
                for j in agent.neighbors:
                    new_sigma += agent.weights[j] * prev_sigmas[j]
                new_sigma += self.phi_i(agent.state) - self.phi_i(prev_states[agent.id])
                agent.sigma_estimate = new_sigma
            ...
            total_grad_norm = np.linalg.norm( sum( (self.gradient_1_cost(agent, agent.state, agent.sigma_estimate) + self.gradient_phi(agent.state) @ agent.v_estimate) for agent in self.agents) )
